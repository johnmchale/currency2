...
function CurrencyCalculator() {
  ...
  const [zarPosts, setZarPosts] = useState([]);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      myHeaders.append('apikey', 'DG8JgIii6PxXugvkdspaz2UVYxNIqfss');
      ...
      fetch(
        'https://api.apilayer.com/currency_data/convert?to=ZAR&from=GBP&amount=1',
        requestOptions
      )
        .then((response) => response.json())
        .then((result) => setZarPosts(result.result));
      //.then((result) => console.log(result.result));

      fetch(
        'https://api.apilayer.com/currency_data/convert?to=GBP&from=ZAR&amount=1',
        requestOptions
      )
        .then((response) => response.json())
        .then((result) => setGbpPosts(result.result));
      //.then((result) => console.log(result.result));
    };
    fetchPosts();
  }, []);
